...
exports.updateDealsData = () => {
  getLatestAssets.then((deals) => {
    console.log("deals updating...");
    fs.writeFileSync("./data/openseaDeals.json", JSON.stringify(deals));
...
const getLatestAssets = new Promise((resolve, reject) => {
  ...
  var repeater = setInterval(function () {
    if (shopIndex < shopdata.length) {
      ...
      dealsList.push({ timestamp: Date.now() });
      resolve(dealsList);
    }
  }, 2200);
});
...
function getTokenValue(orders, token) {
  var tokenOrders = orders.filter(
    (order) => order.payment_token_contract.symbol == token && order.side == 1
  );
